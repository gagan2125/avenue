...
export default function SalesTab({ eventId, event }) {
  ...
  const handleTicketUpdate = async () => {
    try {
      const updatedData = {
        paymentId: selectedTicketUpdate._id,
        updateData: {
          firstName: selectedTicketUpdate.firstName,
          email: selectedTicketUpdate.email
        }
      }
      const response = await axios.put(`${url}/payment/update-payment`, updatedData)
            ...
            <div className="flex flex-col gap-4 p-6">
              <div className="flex flex-col items-start justify-between gap-4">
                ...
                <div className="flex flex-col gap-3 w-full">
                  ...
                  <input
                    type="text"
                    value={selectedTicketUpdate?.user_id?.phoneNumber || ""}
                    readOnly
                    placeholder="+1 (555) 203 2587"
                    className="border bg-primary text-white text-sm border-white/10 h-10 rounded-lg px-5 py-2.5 focus:outline-none w-full disabled:bg-white/10 disabled:text-gray-300"
      ...
      {editNotificationModal && (
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm">Ticket updated successfully</p>
          </div>
          <button
