...
function TrafficSources({ eventId }) {
    ...
    const [manualVisitCount, setManualVisitCount] = useState(0);
        ...
        const fetchData = async () => {
            ...
            try {
                const [gaResponse, manualResponse] = await Promise.all([
                    axios.get(
                        `${url}/analytics/traffic-sources?event_id=${eventId}`,
                    ),
                    axios.get(`${url}/visit/get-visit/${eventId}`),
                ]);
                ...
                if (manualResponse.data && manualResponse.data.data) {
                    manualCount = parseInt(manualResponse.data.data.count || 0);
                    setManualVisitCount(manualCount);
                }
                ...
                if (directIndex >= 0) {
                    combinedSources[directIndex] = {
                        ...combinedSources[directIndex],
                        sessions:
                            combinedSources[directIndex].sessions + manualCount,
                    };
                ...
                combinedSources = combinedSources.map((source) => ({
                    ...
                    percentage:
                        combinedTotal > 0
                            ? ((source.sessions / combinedTotal) * 100).toFixed(
                                  2,
                              ) + "%"
                            : "0%",
                }));
                ...
                const formattedData = combinedSources.map((source, index) => {
                    const fill =
                        colorMap[source.source.toLowerCase()] ||
                    ...
                    return {
                        ...
                        isManual:
                            source.source.toLowerCase() === "direct" &&
                            manualCount > 0,
                    };
                });
                ...
                try {
                    const manualResponse = await axios.get(
                        `${url}/visit/get-visit/${eventId}`,
                    );
                    if (manualResponse.data && manualResponse.data.data) {
                        const manualCount = parseInt(
                            manualResponse.data.data.count || 0,
                        );
                        setManualVisitCount(manualCount);
                    ...
                    console.error(
                        "Error fetching manual visit data:",
                        manualError,
                    );
                }
            } finally {
        ...
        if (active && payload && payload.length) {
            return (
                <div className="bg-[#111] p-2 border border-white/10 rounded">
                    ...
                    <p className="text-white text-sm">{`Sessions: ${payload[0].value}`}</p>
                    <p className="text-white text-sm">{`${payload[0].payload.percentage}% of total`}</p>
                </div>
            );
        }
        return null;
